...
uint8_t nodeDeviceEUI[8] = {0x70, 0xB3, 0xD5, 0x7E, 0xD0, 0x06, 0xF6, 0xAA};
/** Application EUI required for network join */
uint8_t nodeAppEUI[8] = {0xD2, 0xBE, 0x81, 0xCF, 0xA5, 0xE9, 0xA1, 0x21};
/** Application key required for network join */
uint8_t nodeAppKey[16] = {0x4F, 0x7B, 0x32, 0x55, 0x67, 0x00, 0x81, 0xBC, 0x75, 0x2B, 0xAA, 0x59, 0xA1, 0x20, 0x5E, 0xC6};
...
bool sendLoRaFrame(float temperature, float humidity, float pressure, float gas_resistance, float lux, float als, float uvi, float uvs)
{
  ...
  uint32_t gas_res = gas_resistance/10;

  uint16_t _lux = (lux*100);
  uint16_t _als = (als*100);
  uint16_t _uvi = (uvi*100);
  uint16_t _uvs = (uvs*100);

  // float lux, float als, float uvi, float uvs
  ...
  m_lora_app_data_buffer[buffSize++] = (uint8_t)((press / 65536) % 256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)((press / 256) % 256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(press % 256);

  
  m_lora_app_data_buffer[buffSize++] = 0x04;
  m_lora_app_data_buffer[buffSize++] = (uint8_t)((gas_res / 65536) % 256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)((gas_res / 256) % 256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(gas_res % 256);


  
  m_lora_app_data_buffer[buffSize++] = 0x05;
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_lux/256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_lux%256);
  
  m_lora_app_data_buffer[buffSize++] = 0x06;
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_als/256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_als%256);
  
  m_lora_app_data_buffer[buffSize++] = 0x07;
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_uvi/256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_uvi%256);
  
  m_lora_app_data_buffer[buffSize++] = 0x08;
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_uvs/256);
  m_lora_app_data_buffer[buffSize++] = (uint8_t)(_uvs%256);
